...
KUKA_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path = "/workspace/isaaclab/source/my_tasks/dual_iiwa_robot.usd",
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    ee_goals = [
        [-0.5+0.5, 0.5+0.5, 0.7+0.5, 0.707, 0, 0.707, 0],
        [-0.5-+0.5, -0.4+0.5, 0.6+0.5, 0.707, 0.707, 0.0, 0.0],
    ...
    if args_cli.robot == "franka_panda":
        ...
        robot_entity_cfg = SceneEntityCfg("robot", joint_names=["left_iiwa_joint_.*"], body_names=["left_iiwa_link_ee"])
    else:
        raise ValueError(f"Robot {args_cli.robot} is not supported. Valid: franka_panda, ur10")
